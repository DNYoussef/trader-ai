...
const TradingTerminal: React.FC<TradingTerminalProps> = ({
  ...
  const generateChartData = () => {
    ...
    // Map timeframe to milliseconds and number of data points
    const TIMEFRAME_CONFIG = {
      '1m': { intervalMs: 60000, numPoints: 60 },       // 1 hour of 1-min bars
      '5m': { intervalMs: 300000, numPoints: 72 },      // 6 hours of 5-min bars
      '15m': { intervalMs: 900000, numPoints: 96 },     // 1 day of 15-min bars
      '1h': { intervalMs: 3600000, numPoints: 168 },    // 1 week of hourly bars
      '4h': { intervalMs: 14400000, numPoints: 180 },   // 1 month of 4-hour bars
      '1d': { intervalMs: 86400000, numPoints: 365 }    // 1 year of daily bars
    };

    const config = TIMEFRAME_CONFIG[activeTimeframe as keyof typeof TIMEFRAME_CONFIG] || TIMEFRAME_CONFIG['15m'];
    const { intervalMs, numPoints } = config;

    // Generate data points based on selected timeframe
    for (let i = 0; i < numPoints; i++) {
      const timestamp = Date.now() - (numPoints - i) * intervalMs;  // ✅ FIXED: Uses selected timeframe
