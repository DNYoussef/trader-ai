        ...
        <table className="min-w-full">
          ...
          <tbody>
            {safePositions.map((position, index) => (
              <tr key={position?.symbol ?? index} className="border-b border-gray-100 dark:border-gray-800">
                <td className="py-3 px-4 font-medium text-gray-900 dark:text-white">{position?.symbol ?? '-'}</td>
                <td className="py-3 px-4 text-right text-gray-600 dark:text-gray-400">{position?.quantity ?? 0}</td>
