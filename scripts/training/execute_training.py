...
def create_sample_data():
    ...
    returns[1200:1400] *= 0.5  # Low volatility period

    prices = 100 * np.exp(np.cumsum(returns))
    volumes = np.random.exponential(1000000, n_samples)

    # Additional features - fixed fillna issue
    price_series = pd.Series(prices)
    ma_5 = price_series.rolling(5).mean()
    ma_20 = price_series.rolling(20).mean()

    # Fill NaN values properly
    ma_5 = ma_5.fillna(price_series.iloc[0])
    ma_20 = ma_20.fillna(price_series.iloc[0])
    ...
    try:
        ...
        for model_name in models_in_registry:
            try:
                ...
                print(f"   - {model_name}: Error getting info - {e}")
        ...
        if successful_models:
            ...
            test_data = data.iloc[:50]  # First 50 rows
            ...
            for model_name in loaded_models:
                try:
                    ...
                    print(f"   - {model_name}: Prediction failed - {str(e)[:100]}")

            # Test ensemble prediction
            if len(loaded_models) > 1:
                try:
                    ensemble_result = predictor.predict_ensemble(test_data)
                    if ensemble_result['ensemble_prediction'] is not None:
                    ...
                    print(f"   - Ensemble prediction failed: {str(e)[:100]}")
        ...
        try:
            ...
            print(f"   - Import test failed: {str(e)[:100]}")
        ...
        try:
            ...
            print(f"   - Data processing test failed: {str(e)[:100]}")
        ...
        if len(successful_models) > 0:
            ...
            print("\nCreated files:")
            print("- trained_models/ (with actual model weights)")
            print("- data/training_data.csv (training dataset)")
            print("- mlruns/ (MLflow tracking)")
            print("- training.log (execution log)")
        else:
